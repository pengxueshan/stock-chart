...
const detail_cell = ['时\u3000间','涨跌幅','价\u3000格','均\u3000价','成交量','成交额'];
const detail_cell_color = ['time','rise','price','avg','volume','amount'];
...
export default class CrossLine{
	...
	update(data){
		...
		let cell_len = detail_cell.length;
		for(let i=0;i<cell_len;i++) {
			let cell_value = null;
			if(i==0) {
				let dateStr = ''+data.item.time;
				cell_value = `${dateStr.slice(8, 10)}:${dateStr.slice(10, 12)}`;
			}
			if(i==1) {
				cell_value = data.item.rise.toFixed(2) + '%';
			}
			if(i==2) {
				cell_value = data.item.price.toFixed(2);
			}
			if(i==3) {
				cell_value = data.item.avg.toFixed(2);
			}
			if(i==4) {
				cell_value = this.fitNumber(data.item.volume);
			}
			if(i==5) {
				cell_value = this.fitNumber(data.item.amount);
			}

			this.sel_detail_text
				.append('tspan')
				.attr('x',cfg.detail_label_left?cfg.x1+5:cfg.x2-cfg.detail_label_width+5)
				.attr('y',cfg.y1+15+i*20)
				.text(detail_cell[i]);
			this.sel_detail_text
				.append('tspan')
				...
				.attr('y',cfg.y1+15+i*20)
				.text(cell_value)
				.attr('fill',this.getColor(detail_cell_color[i],data.item));
		}
	}
